...
Ifx_TickTime            g_ticksForSTM;                          /* Variable to store the number of ticks to wait    */
...
void isrSTM(void)
{
    ...
    IfxStm_increaseCompare(STM, g_STMConf.comparator, g_ticksForSTM);
    transmitCanMessage_CAN0();      /* Transmit the CAN message */
}



/* Function to initialize the STM */
void initSTM(int msPeriod)
{
    /* Initialize time constant based on the timer period passed*/
    g_ticksForSTM = IfxStm_getTicksFromMilliseconds(BSP_DEFAULT_TIMER, msPeriod);
    ...
    g_STMConf.ticks = g_ticksForSTM;                /* Set the number of ticks after which the timer triggers an
